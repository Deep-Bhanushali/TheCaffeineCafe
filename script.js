...
const carouselContainer = document.querySelector("#carousel-section .relative");
...
function renderGallery() {
  ...
  container.innerHTML = galleryImages
    .map(
      (image, index) => `
        <div class="overflow-hidden rounded-lg shadow-lg cursor-pointer transform transition-transform duration-300 hover:scale-105" onclick="openLightbox(${index})">
          <img src="${image.thumbnail}" alt="${image.alt}" class="w-full h-64 object-cover">
        </div>
      `
    )
    .join("");
